...
class TestCollectorSum:
    ...
    @pytest.mark.parametrize(Params.sig_iterable, Params.iterable_multi())
    def test_aggregate_hasMultipleItems_customSelector(self, iter_type, iterable, iterable_list):
        if iter_type == 'str':
            # sum should be used with numbers only
            return
    ...
    def test_aggregate_hasSingleItem_customObjects(self, iter_type, iterable):
        assert q(iterable).sum(by=lambda o: o.a) == 0

    @pytest.mark.parametrize(Params.sig_iterable_obj, Params.iterable_obj_multi())
    def test_aggregate_hasMultipleItems_customObjects(self, iter_type, iterable):
        assert q(iterable).sum(by=lambda o: o.a) == 10

    @pytest.mark.parametrize(Params.sig_iterable_obj, Params.iterable_obj_single())
    def test_aggregate_hasSingleItem_customObjects_customAccumulator(self, iter_type, iterable):
        assert q(iterable).sum(accumulator=MyTestClass(0, 0)) == MyTestClass(0, 0)

    @pytest.mark.parametrize(Params.sig_iterable_obj, Params.iterable_obj_multi())
    def test_aggregate_hasMultipleItems_customObjects_customAccumulator(self, iter_type, iterable):
        assert q(iterable).sum(accumulator=MyTestClass(0, 0)) == MyTestClass(10, 10 * 2)

    @pytest.mark.parametrize(Params.sig_iterable_obj, Params.iterable_obj_multi())
    def test_aggregate_hasMultipleItems_customObjectsNoAccumulator(self, iter_type, iterable):
